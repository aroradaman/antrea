...
func PrepareHostInterfaceConnection(
	...
	interfaceIndex int,
	interfaceOFPort int32,
	interfaceMAC net.HardwareAddr,
	interfaceIPs []*net.IPNet,
	...
	if err := RenameInterface(interfaceName, bridgedName); err != nil {
		return "", false, err
	}
	if _, err := bridge.CreateInternalPort(interfaceName, interfaceOFPort, interfaceMAC.String(), externalIDs); err != nil {
	...
	if err := wait.PollImmediate(100*time.Millisecond, 10*time.Second, func() (bool, error) {
		...
		if err != nil {
			klog.V(4).InfoS("OVS bridge local port is not ready", "port", interfaceName, "err", err)
			return false, nil
		}
	...
	localLink, err := netlink.LinkByName(interfaceName)
	if err != nil {
		return "", false, err
	}
	if _, _, err = SetLinkUp(interfaceName); err != nil {
	...
	for _, ip := range interfaceIPs {
		if ip.IP.To4() == nil {
			klog.InfoS("Interface has IPv6 address, ensuring that IPv6 is enabled on bridge local port", "port", interfaceName)
			if err := EnsureIPv6EnabledOnInterface(interfaceName); err != nil {
				klog.ErrorS(err, "Failed to ensure that IPv6 is enabled on bridge local port, moving uplink IPs to bridge is likely to fail", "port", interfaceName)
			}
			break
		}
	}

	if err = ConfigureLinkAddresses(localLink.Attrs().Index, interfaceIPs); err != nil {
		return "", false, err
	}
	if err = ConfigureLinkAddresses(interfaceIndex, nil); err != nil {
		return "", false, err
	}
	...
	if err = ConfigureLinkRoutes(localLink, interfaceRoutes); err != nil {
		return "", false, err
	}
...
func RestoreHostInterfaceConfiguration(brName string, interfaceName string) {
	...
	if interfaceName != "" {
		...
		if exists, err := interfaceExists(bridgedName); exists || err != nil {
			...
			if exists, err = interfaceExists(interfaceName); exists || err != nil {
